...
static volatile int* _PORTE = (volatile int*) 0xbf886110;
...
void user_isr()
{
    ...
    IFS(0) &= 0xeff; // timer2 interrupt flag status, zero index 8
}
...
void __labwork()
{
    ...
    if (buttons != 0) // some button is pressed
    {
        ...
        (*_PORTE) = (*_PORTE) & 0xffffff00 | (seconds % 256);
    ...
    else
    {
        ...
        if (timeoutcount == 0)
        {
            tick( &mytime );
            // update the LEDs
            (*_PORTE) = ((*_PORTE) & 0xffffff00) | (((*_PORTE) & 0xff) + 1);
